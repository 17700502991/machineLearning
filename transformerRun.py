...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        pe = torch.zeros(max_len, d_model) # 一个全部元素设置为0的张量
        # pe [max_len,d_model]
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)
        # arange创建一个一维张量，unsqueeze增加一个维度
        # position  [max_len，1]
    ...
    def forward(self, x):
        ...
        return self.dropout(x) # 丢弃一些神经元，其实就是将tensor x中的某些值设置为0
    # ***********************************************#


# 由于在 Encoder 和 Decoder 中都需要进行 mask 操作，这句话就是错的吧，输入完全不需要mask,不懂啊
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.src_emb = nn.Embedding(src_vocab_size, d_model) # 这个里面也是可学习的参数
...
optimizer = optim.SGD(model.parameters(), lr=1e-3, momentum=0.99) # 参数为学习率和动量
# ***********************************************#
for epoch in range(30):
    ...
    loss = criterion(outputs, dec_outputs.view(-1)) # 这个里面有sorfmax和负对数似然
    print('Epoch:', '%04d' % (epoch + 1), 'loss =', '{:.6f}'.format(loss))

    optimizer.zero_grad() # 梯度清零
    loss.backward() # 反向传播，计算梯度并存储梯度于参数的.grad属性中
    optimizer.step() # 更新参数
